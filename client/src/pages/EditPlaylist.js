...
const EditPlaylist = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        setFormData({
          ...
          isPublic: playlistRes.data.isPublic
        });
        setSelectedSongs(playlistRes.data.songs.map(song => song._id));
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: error.response?.data?.message || 'Failed to update playlist'
      });
    } finally {
      setSaving(false);
    }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setMessage({
        type: 'error',
        text: error.response?.data?.message || 'Failed to delete playlist'
      });
    } finally {
      setSaving(false);
    }
      ...
      <form onSubmit={handleSubmit} className="max-w-2xl space-y-6">
        ...
        <div className="pt-6 border-t border-white/10">
          <button
            ...
            disabled={saving}
            className="w-full py-2 px-4 bg-red-500/10 text-red-500 rounded-lg hover:bg-red-500/20 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Delete Playlist
          </button>
        </div>
